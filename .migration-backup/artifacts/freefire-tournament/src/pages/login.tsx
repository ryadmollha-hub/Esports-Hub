...
export default function LoginPage() {
  ...
  useEffect(() => { fetchCaptcha().then(setCaptcha).catch(() => {}); }, []);
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-3 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="text-center mb-3.5">
          ...
          <h1 className="text-2xl font-black uppercase text-white">{t("login_title")}</h1>
          <p className="text-[#a0a0b0] text-sm mt-0.5">{t("login_subtitle")}</p>
        </div>

        <div className="bg-[#12121a] rounded-2xl border border-[#ff6b00]/20 p-3.5 shadow-[0_0_30px_rgba(255,107,0,0.08)]">
          {error && (
            <div className="flex items-center gap-1.5 bg-[#ff2244]/10 border border-[#ff2244]/30 text-[#ff2244] rounded-xl px-2.5 py-1.5 mb-3 text-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5">
            <div>
              <label className="block text-[#a0a0b0] text-xs uppercase tracking-wider font-bold mb-1">{t("login_email")}</label>
            ...
            <div>
              <label className="block text-[#a0a0b0] text-xs uppercase tracking-wider font-bold mb-1">{t("login_password")}</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#a0a0b0] hover:text-white transition-colors">
            ...
            <div className="text-right">
              <Link href="/forgot-password" className="text-xs text-[#ff6b00] hover:text-[#ff8533] transition-colors">{t("login_forgot")}</Link>
            </div>
            ...
            <div className="bg-[#1a1a24] border border-[#2a2a36] rounded-xl p-2.5">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-1 text-xs text-[#a0a0b0] font-bold uppercase tracking-wider">
                  <Shield className="w-3 h-3 text-[#ff6b00]" /> {t("login_security")}
                </div>
              ...
              {captcha ? (
                ...
                <div className="text-[#a0a0b0] text-sm animate-pulse">{t("loading")}</div>
              )}
            </div>

            <button type="submit" disabled={loading || !captcha}
              className="w-full py-2 bg-[#ff6b00] text-white font-black uppercase text-sm rounded-xl hover:bg-[#e66000] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-[0_0_15px_rgba(255,107,0,0.3)] mt-1">
          ...
          <div className="mt-3 text-center text-sm text-[#a0a0b0]">
            {t("login_no_account")}{" "}
            <Link href="/sign-up" className="text-[#ff6b00] font-bold hover:text-[#ff8533] transition-colors">{t("login_register_now")}</Link>
          </div>
        </div>

        <div className="mt-2.5 text-center">
          <Link href="/admin-login" className="inline-flex items-center gap-1 text-xs text-[#606070] hover:text-[#a0a0b0] transition-colors">
