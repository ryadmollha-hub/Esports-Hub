...
export default function TeamsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <Navbar />
      <div className="max-w-7xl mx-auto px-3 pt-16 pb-16">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          ...
          {isSignedIn && (
            <Link
              ...
              className="px-3.5 py-2 bg-[#ff6b00] text-white font-bold uppercase text-sm rounded-xl hover:bg-[#e66000] transition-all shadow-[0_0_15px_rgba(255,107,0,0.3)] flex items-center gap-1.5"
            >
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full max-w-md pl-7 pr-3 py-2.5 bg-[#12121a] border border-[#2a2a36] rounded-xl text-white placeholder-[#a0a0b0] focus:outline-none focus:border-[#ff6b00] transition-colors"
          />
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <Users className="w-16 h-16 mx-auto mb-3 text-[#ff6b00]/30" />
            <h3 className="text-xl font-bold mb-1.5">No teams found</h3>
            <p className="text-[#a0a0b0] mb-4">Be the first to create a team!</p>
            {isSignedIn && (
              <Link href="/teams/my" data-testid="link-create-team" className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-[#ff6b00] text-white font-bold uppercase rounded-xl hover:bg-[#e66000] transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {(teams as any[]).map((team: any) => (
              <div key={team.id} className="bg-[#12121a] rounded-xl border border-[#ff6b00]/20 hover:border-[#ff6b00]/40 transition-all p-3.5" data-testid={`card-team-${team.id}`}>
                ...
                {isSignedIn && (
                  <button
                    ...
                    className="w-full py-1.5 text-sm font-bold uppercase bg-[#ff6b00]/10 border border-[#ff6b00]/30 text-[#ff6b00] rounded-lg hover:bg-[#ff6b00]/20 transition-colors disabled:opacity-50"
                  >
                    Request to Join
                  </button>
                )}
