...
export default function BottomNav() {
  ...
  const NavLink = ({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) => {
    ...
    return (
      <Link
        href={href}
        className={`flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all min-w-[44px] ${
        ...
        <div className="flex items-center justify-around px-1 py-1 max-w-lg mx-auto">
          {!isLoggedIn ? (
            ...
            <>
              ...
              <NavLink href="/tournaments" icon={Trophy} label="Tournament" />
              <NavLink href="/wallet" icon={Wallet} label="Wallet" />

              {/* + Create Match */}
              <button
                onClick={openCreateMatch}
                className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all min-w-[44px] group"
              ...
              >
                ...
                <span className="text-[9px] font-bold uppercase tracking-wide text-[#ff6b00]">Create</span>
              ...
              <button
                ...
                className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl text-[#606070] hover:text-[#a0a0b0] transition-all min-w-[44px]"
              >
