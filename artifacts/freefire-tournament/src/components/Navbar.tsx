...
import { Menu, X, Flame, Shield, LayoutDashboard, LogOut, User } from "lucide-react";
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          {isUser && (
            <>
              ...
              >
                ...
                {user?.displayName ?? user?.username ?? "Dashboard"}
              </Link>
              <button
          ...
          {!isUser && !isAdmin && (
            <>
              ...
              <Link href="/admin-login" className="flex items-center gap-1 px-2 py-1.5 text-xs font-bold uppercase text-[#a0a0b0]/50 hover:text-[#ff6b00]/70 transition-colors">
      ...
      {open && (
        <div className="lg:hidden bg-[#0a0a0f] border-t border-[#ff6b00]/20 px-4 py-4 flex flex-col gap-3">
          ...
          <div className="border-t border-[#ff6b00]/10 pt-3 flex flex-col gap-3">
            {isAdmin && (
              <Link href="/admin" onClick={() => setOpen(false)} className="flex items-center gap-2 text-sm font-black uppercase text-[#ff6b00]">
            ...
            {isUser && (
              <>
                <Link href="/dashboard" onClick={() => setOpen(false)} className="flex items-center gap-2 text-sm font-medium uppercase text-[#a0a0b0]">
                  <LayoutDashboard className="w-4 h-4" /> Dashboard
                </Link>
                <button onClick={handleLogout} className="flex items-center gap-2 text-sm font-medium uppercase text-[#a0a0b0]">
                  <LogOut className="w-4 h-4" /> Logout
                </button>
              </>
            )}
            ...
            {isAdminSession && !isUser && (
              <button onClick={handleAdminLogout} className="flex items-center gap-2 text-sm font-medium uppercase text-[#a0a0b0]">
                <LogOut className="w-4 h-4" /> Log Out
              </button>
            )}
