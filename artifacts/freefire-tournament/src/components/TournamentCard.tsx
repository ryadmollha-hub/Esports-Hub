...
// IMPORTANT: this hook must NEVER auto-declare "live" or "completed" purely
// from wall-clock time — those are authoritative backend states (matchLive /
// resultsPublished) and must only ever be reflected once the server actually
// reports them (see useMatchStatus below). Doing otherwise was the root cause
// of "Match Live appears automatically" and related state-overlap bugs. The
// only thing the clock is allowed to drive here is the purely decorative
// "starting_soon" label on the countdown.
function useClientStatus(serverStatus: string, startDate: string): StatusKey {
  const computeStatus = (): StatusKey => {
    ...
    if (now >= start - 10 * 60 * 1000 && now < start) return "starting_soon";
    return "upcoming";
  };
...
/**
 * Polls real match data (never inferred from time) to learn whether a room has
 * been released or a match has actually gone live. `activeMatchNumbers` is
 * scoped to only matches worth flagging (live / room released) so the badge
 * row doesn't fill up with every match ever created for the tournament.
 */
function useMatchStatus(tournamentId: number, isTerminal: boolean): { roomOpen: boolean; matchLive: boolean; activeMatchNumbers: number[] } {
  const [state, setState] = useState<{ roomOpen: boolean; matchLive: boolean; activeMatchNumbers: number[] }>({
    roomOpen: false,
    matchLive: false,
    activeMatchNumbers: [],
  });
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      // Both flags come straight from the server's `effectiveStatus`
      // (computeMatchVisibility), which is itself driven only by the
      // explicit roomReleased / matchLive database columns — never by time.
      const roomOpen  = data.some(m => m.status === "room_released");
      const matchLive = data.some(m => m.status === "live");
      const activeMatchNumbers = data
        .filter(m => m.status === "room_released" || m.status === "live")
        .map(m => m.matchNumber)
        .filter(Boolean);
      setState({ roomOpen, matchLive, activeMatchNumbers });
    } catch {}
  }, [tournamentId, isTerminal]);
...
export default function TournamentCard({ t, featured = false }: { t: Tournament; featured?: boolean }) {
  ...
  const isTerminal = isCancelled || t.resultsPublished === true || t.status === "completed" || t.status === "ended";
  const { roomOpen, matchLive, activeMatchNumbers } = useMatchStatus(t.id, isTerminal);

  // Derive effective status. Every branch here reflects an explicit,
  // independent backend flag — never inferred from another state or from time:
  // Priority (highest → lowest):
  //   1. cancelled / resultsPublished / server completed-ended → terminal states
  //   2. matchLive (real matchesTable.matchLive)                → live
  //   3. roomOpen (real matchesTable.roomReleased)               → room_open
  //   4. client-computed "starting_soon"/"upcoming" (decorative only)
  const effectiveStatus: StatusKey = isCancelled
    ? "cancelled"
    : isTerminal
  ...
  const isLive = effectiveStatus === "live" || effectiveStatus === "ongoing" || effectiveStatus === "room_open";
