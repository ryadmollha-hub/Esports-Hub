...
export default function DashboardPage() {
  ...
  // The initial fetch above only runs once per tournament (guarded by the
  // `!matchesByTournament[...]` check), so without this it never learns that
  // a room was released, a match went live, etc. — the player would have to
  // hard-refresh the whole page to see it. Poll the same approved
  // tournaments' matches periodically so room-release (and other) state
  // changes made by the admin reach this page on their own.
  useEffect(() => {
    const approvedIds = regs.filter((r: any) => r.status === "approved").map((r: any) => r.tournamentId);
    if (approvedIds.length === 0) return;
    const intervalId = setInterval(() => {
      approvedIds.forEach((tournamentId: number) => fetchMatchesForTournament(tournamentId));
    }, 15000);
    return () => clearInterval(intervalId);
  }, [regs, fetchMatchesForTournament]);
