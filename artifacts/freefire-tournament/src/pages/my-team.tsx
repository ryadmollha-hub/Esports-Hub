  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#0a0a0f]">
        <Navbar />
        <div className="max-w-4xl mx-auto px-3 pt-16">
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="max-w-4xl mx-auto px-3 pt-16 pb-20">
        <h1 className="text-2xl sm:text-4xl font-black uppercase mb-1.5" data-testid="heading-my-team">
          My <span className="text-[#ff6b00]">Team</span>
        </h1>

        {!hasTeam ? (
          <div className="mt-5">
            <p className="text-[#a0a0b0] mb-5">You don't have a team yet. Create one or request to join an existing team.</p>
            <div className="bg-[#12121a] rounded-2xl border border-[#ff6b00]/20 p-5 max-w-lg">
              <h2 className="text-xl font-black uppercase mb-4 text-[#ff6b00]">Create a Team</h2>
              <form onSubmit={form.handleSubmit(onCreateTeam)} className="space-y-3">
                <div>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wide text-[#a0a0b0] mb-1">Tag <span className="font-normal normal-case text-xs">(max 6 chars)</span></label>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wide text-[#a0a0b0] mb-1">Your Free Fire UID</label>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wide text-[#a0a0b0] mb-1">Your In-Game Name</label>
                ...
                <button type="submit" disabled={createTeam.isPending} data-testid="button-create-team"
                  className="w-full py-2.5 bg-[#ff6b00] text-white font-black uppercase rounded-xl hover:bg-[#e66000] disabled:opacity-50 transition-all shadow-[0_0_20px_rgba(255,107,0,0.3)]">
          ...
          <div className="mt-4 space-y-4">
            {/* Team Header */}
            <div className="bg-[#12121a] rounded-2xl border border-[#ff6b00]/20 p-4">
              ...
              {/* Captain actions */}
              <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-[#ff6b00]/10">
                {isCaptain && (
                  <button
                    onClick={() => setShowEditForm((v) => !v)}
                    className="flex items-center gap-1 px-2.5 py-1 bg-[#ff6b00]/10 border border-[#ff6b00]/30 text-[#ff6b00] rounded-lg text-xs font-bold uppercase hover:bg-[#ff6b00]/20 transition-colors"
                  >
                ...
                {isCaptain && (
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 bg-[#ff2244]/10 border border-[#ff2244]/30 text-[#ff2244] rounded-lg text-xs font-bold uppercase hover:bg-[#ff2244]/20 transition-colors disabled:opacity-50"
                  >
              ...
              {showEditForm && isCaptain && (
                <form onSubmit={editForm.handleSubmit(onEditTeam)} className="mt-3 pt-3 border-t border-[#ff6b00]/10 space-y-2.5">
                  <h3 className="text-sm font-black uppercase text-[#ff6b00] mb-2.5">Edit Team Details</h3>
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wide text-[#a0a0b0] mb-1">Team Name</label>
                      <input {...editForm.register("name")}
                        className="w-full px-2.5 py-2 bg-[#1a1a24] border border-[#2a2a36] rounded-lg text-white text-sm placeholder-[#a0a0b0] focus:outline-none focus:border-[#ff6b00] transition-colors" />
                    ...
                    <div>
                      ...
                      <input {...editForm.register("tag")} placeholder="e.g. FIRE"
                        className="w-full px-2.5 py-2 bg-[#1a1a24] border border-[#2a2a36] rounded-lg text-white text-sm placeholder-[#a0a0b0] focus:outline-none focus:border-[#ff6b00] transition-colors font-mono" />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select {...editForm.register("maxMembers")}
                      className="w-full px-2.5 py-2 bg-[#1a1a24] border border-[#2a2a36] rounded-lg text-white text-sm focus:outline-none focus:border-[#ff6b00] transition-colors">
                      <option value={2}>2 (Duo)</option>
                      <option value={4}>4 (Squad)</option>
                      <option value={6}>6</option>
                      <option value={10}>10</option>
                  ...
                  <div className="flex gap-1.5">
                    <button type="submit" disabled={savingEdit}
                      className="flex items-center gap-1 px-3 py-1.5 bg-[#ff6b00] text-white font-bold text-sm uppercase rounded-lg hover:bg-[#e66000] disabled:opacity-50 transition-all">
                      <Save className="w-3.5 h-3.5" />
                      {savingEdit ? "Saving..." : "Save Changes"}
                    </button>
                    <button type="button" onClick={() => setShowEditForm(false)}
                      className="px-3 py-1.5 bg-[#1a1a24] text-[#a0a0b0] font-bold text-sm uppercase rounded-lg hover:text-white transition-colors">
                      Cancel
                    </button>
                  </div>
                </form>
            ...
            {/* Members */}
            <div className="bg-[#12121a] rounded-2xl border border-[#ff6b00]/20 p-4">
              <h3 className="font-black uppercase text-sm text-[#a0a0b0] mb-3 tracking-wider flex items-center gap-1.5">
                <Users className="w-4 h-4" /> Members ({activeMembers.length}/{team.maxMembers ?? 4})
              </h3>
              <div className="space-y-2.5">
                {activeMembers.map((member: any) => (
                  <div key={member.id} className="flex items-center gap-2.5 py-1.5 border-b border-[#ff6b00]/5 last:border-0" data-testid={`row-member-${member.id}`}>
                    ...
                    <span className={`text-xs font-bold uppercase px-1.5 py-0.5 rounded ${member.role === "captain" ? "text-[#ffd700] bg-[#ffd700]/10" : "text-[#a0a0b0] bg-[#1a1a24]"}`}>
                      {member.role}
                    </span>
                    ...
                    {isCaptain && member.role !== "captain" && (
                      <button
                        ...
                        className="p-1 bg-[#ff2244]/10 border border-[#ff2244]/20 rounded-lg text-[#ff2244] hover:bg-[#ff2244]/20 transition-colors disabled:opacity-50"
                      >
                ...
                {activeMembers.length === 0 && (
                  <p className="text-[#a0a0b0] text-sm text-center py-3">No active members yet.</p>
                )}
              </div>
            </div>
            ...
            {isCaptain && (joinRequests as any[]).length > 0 && (
              <div className="bg-[#12121a] rounded-2xl border border-yellow-400/20 p-4">
                <h3 className="font-black uppercase text-sm text-yellow-400 mb-3 tracking-wider">
                  Pending Join Requests ({(joinRequests as any[]).length})
                </h3>
                <div className="space-y-2.5">
                  {(joinRequests as any[]).map((req: any) => (
                    <div key={req.id} className="flex items-center gap-2.5 py-1.5 border-b border-yellow-400/5 last:border-0" data-testid={`row-join-request-${req.id}`}>
                      <div className="flex-1">
