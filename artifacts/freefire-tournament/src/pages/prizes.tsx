    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-24 pb-16">
        ...
        {isLoading ? (
          ...
          <>
            {/* Total prize banner */}
            <div className="relative rounded-2xl overflow-hidden border border-[#ffd700]/30 bg-gradient-to-r from-[#ffd700]/10 to-[#ff6b00]/10 p-8 mb-10 text-center">
            ...
            {t.prizes && t.prizes.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {t.prizes.map((prize: any, i: number) => (
                  <div
                    key={prize.id}
                    className={`rounded-2xl border bg-gradient-to-b p-6 text-center ${prizeColors[i] ?? "from-[#1a1a24] to-[#12121a] border-[#2a2a36] text-white"}`}
                    data-testid={`card-prize-${prize.id}`}
                  >
                    <Trophy className={`w-10 h-10 mx-auto mb-3 ${prizeColors[i]?.split(" ")[3] ?? "text-white"}`} />
                    <div className="text-xl font-black mb-1" data-testid={`text-prize-card-rank-${prize.id}`}>{prize.rank}</div>
                    <div className="text-3xl font-black mb-2" data-testid={`text-prize-card-amount-${prize.id}`}>
