...
import { Search, Trophy, Plus, Swords, Clock, Lock, X } from "lucide-react";
          ...
          {communityLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {communityMatches.map((m: any) => {
                ...
                const canJoin = !isFull && !isPast;
                return (
                  <div key={m.id} className={`bg-[#12121a] rounded-2xl border p-4 transition-colors ${!canJoin ? "border-[#2a2a36] opacity-60" : "border-[#ff6b00]/10 hover:border-[#ff6b00]/30"}`}>
                    {/* Title row */}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs">
                        Fee: <span className="font-black text-[#ff6b00]">{Number(m.entryFee) > 0 ? `৳${Number(m.entryFee).toLocaleString()}` : "Free"}</span>
                      </div>
                      {isPast ? (
                        <span className="text-[#606070] text-xs font-bold">Ended</span>
                      ) : isFull ? (
